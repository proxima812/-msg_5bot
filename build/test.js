...
const session_1 = require("@grammyjs/session"); // Подключаем плагин для сессий
...
const bot = new grammy_1.Bot("7695527442:AAF3oZNuZjMMBpKe4TvrIz_YNtBcVivH9os");
// Подключаем middleware для сессий
bot.use((0, session_1.session)());
// Главное меню
...
bot.on("message:text", async (ctx) => {
    ...
    if (session.step === "name") {
        ...
        await ctx.reply(`Вы указали название: ${session.groupName}. Теперь выберите формат группы.`);
    ...
    // Если пользователь на этапе выбора формата
        ...
        await ctx.reply(`Вы выбрали формат: ${session.groupFormat}. Теперь введите ссылку на группу или сайт.`);
